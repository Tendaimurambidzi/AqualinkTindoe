...
async function loadMeetingFilesFromFirestore(liveId) {
  if (!adminReady || !liveId) return [];
  try {
    ...
    meetingFilesByLive.set(String(liveId), items);
    return items;
  } catch (e) {
    console.warn('Failed to load live files from Firestore:', e?.message || e);
    return [];
  }
}
...
// -------------------- Live meeting files --------------------
app.get('/meeting/files', async (req, res) => {
  const liveId = String(req.query?.liveId || '').trim();
  if (!liveId) return bad(res, 'Missing liveId');

  let items = meetingFilesByLive.get(liveId) || [];
  if (!items.length && adminReady) {
    items = await loadMeetingFilesFromFirestore(liveId);
  }
...
app.post('/meeting/files/create', async (req, res) => {
  try {
    const liveId = String(req.body?.liveId || '').trim();
    const name = String(req.body?.name || '').trim();
    const uploadedBy = String(req.body?.uploadedBy || '').trim();
    const downloadUrl = String(req.body?.downloadUrl || '').trim();
    if (!liveId || !name || !uploadedBy || !downloadUrl) {
      return bad(res, 'Missing liveId/name/uploadedBy/downloadUrl');
    }

    const fileId = `mf_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const item = {
      id: fileId,
      liveId,
      name,
      ...
      storagePath: String(req.body?.storagePath || ''),
      downloadUrl,
      uploadedBy,
      uploadedByName: String(req.body?.uploadedByName || 'Someone'),
      createdAt: Date.now(),
      pinned: false,
      pinnedAt: 0,
    };
    const existing = meetingFilesByLive.get(liveId) || [];
    meetingFilesByLive.set(liveId, [item, ...existing]);
    ...
    if (adminReady) {
      try {
        ...
        console.warn('Failed to persist shared file to Firestore:', e?.message || e);
      }
    }

    roomBroadcast(liveId, { type: 'file_shared', file: item });
    return res.json({ ok: true, item });
  } catch (e) {
    console.error('meeting/files/create error', e);
    return bad(res, 'Failed to create meeting file', 500);
  }
});

app.post('/meeting/files/delete', async (req, res) => {
  try {
    const liveId = String(req.body?.liveId || '').trim();
    const fileId = String(req.body?.fileId || '').trim();
    const requesterUid = String(req.body?.requesterUid || '').trim();
    const isHost = !!req.body?.isHost;
    const isCoHost = !!req.body?.isCoHost;
    if (!liveId || !fileId || !requesterUid) {
      return bad(res, 'Missing liveId/fileId/requesterUid');
    }

    const current = meetingFilesByLive.get(liveId) || (await loadMeetingFilesFromFirestore(liveId));
    const target = current.find((row) => String(row.id) === fileId);
    if (!target) return bad(res, 'File not found', 404);
    const canDelete = isHost || isCoHost || String(target.uploadedBy) === requesterUid;
    if (!canDelete) return bad(res, 'Not allowed', 403);

    const next = current.filter((row) => String(row.id) !== fileId);
    meetingFilesByLive.set(liveId, next);
    ...
    if (adminReady) {
      try {
        ...
        console.warn('Failed to delete shared file in Firestore:', e?.message || e);
      }
    }

    roomBroadcast(liveId, { type: 'file_deleted', fileId, requesterUid });
    return res.json({ ok: true, deletedFileId: fileId });
  } catch (e) {
    console.error('meeting/files/delete error', e);
    return bad(res, 'Failed to delete meeting file', 500);
  }
});

app.post('/meeting/files/pin', async (req, res) => {
  try {
    const liveId = String(req.body?.liveId || '').trim();
    const fileId = String(req.body?.fileId || '').trim();
    const requesterUid = String(req.body?.requesterUid || '').trim();
    ...
    if (!isHost && !isCoHost) return bad(res, 'Only host/co-host can pin files', 403);

    const current = meetingFilesByLive.get(liveId) || (await loadMeetingFilesFromFirestore(liveId));
    if (!current.length) return bad(res, 'No shared files found', 404);
    const now = Date.now();
    const next = current.map((row) => {
      const pinned = String(row.id) === fileId;
      return {
        ...row,
        pinned,
    ...
    const pinnedItem = next.find((row) => String(row.id) === fileId);
    if (!pinnedItem) return bad(res, 'File not found', 404);
    meetingFilesByLive.set(liveId, next);
    ...
    if (adminReady) {
      try {
        ...
        next.forEach((row) => {
          ...
          batch.set(
            ref,
            { pinned: !!row.pinned, pinnedAt: Number(row.pinnedAt || 0) },
            { merge: true },
          );
        });
        await batch.commit();
      } catch (e) {
        console.warn('Failed to pin shared file in Firestore:', e?.message || e);
      }
    }

    roomBroadcast(liveId, { type: 'file_pinned', fileId, requesterUid });
    return res.json({ ok: true, item: pinnedItem });
  } catch (e) {
    console.error('meeting/files/pin error', e);
    return bad(res, 'Failed to pin meeting file', 500);
  }
});
