...
const ui = {
  ...
  radius: {
    md: appTokens.radius.md,
    lg: appTokens.radius.lg,
    xl: appTokens.radius.xl,
  },
  spacing: {
  ...
  type: {
    title: appTokens.type.title,
    body: appTokens.type.body,
    caption: appTokens.type.caption,
    meta: appTokens.type.meta,
  },
};
...
interface MainFeedItemProps {
  ...
  setExpandedPosts: React.Dispatch<
  ...
  setEchoExpansionInProgress: React.Dispatch<
    React.SetStateAction<Record<string, boolean>>
  >;
  setExpandedEchoes: React.Dispatch<
    React.SetStateAction<Record<string, boolean>>
  >;
  setEchoesPageSize: React.Dispatch<
...
const MainFeedItem = memo<MainFeedItemProps>(
  ({
    ...
    const [activeEchoActionId, setActiveEchoActionId] = useState<string | null>(
      null,
    );
    const [localEchoHugs, setLocalEchoHugs] = useState<
      Record<string, { hugs: number; hugged: boolean }>
    >({});
    ...
    const [lastSplashAction, setLastSplashAction] = useState<
      'add' | 'remove' | null
    >(null);
    const [preferFallbackVideoSource, setPreferFallbackVideoSource] =
      useState(false);
    ...
    useEffect(() => {
      ...
      const postFallbackTs =
        (item as any)?.createdAt || (item as any)?.timestamp || new Date();
      ...
      const refreshStatus = () => {
        const mostRecentLastSeen = chooseMostRecent(
          firestoreLastSeen,
          rtdbLastSeen,
        );
        const mostRecentActive = chooseMostRecent(
      ...
      const unsubscribeFs = firestore()
        ...
        .onSnapshot(doc => {
          ...
          firestoreLastActiveAt =
            data?.lastActiveAt ||
            data?.lastHeartbeat ||
            userData[ownerUid]?.lastActiveAt ||
            null;
          refreshStatus();
        });
    ...
    const isAnyModalOpen =
      showMakeWaves || showAudioModal || !!capturedMedia || showLive;
    ...
    const galleryMediaItems = useMemo(() => {
      ...
      if (
        Array.isArray((item as any).galleryItems) &&
        (item as any).galleryItems.length > 0
      ) {
    ...
    useEffect(() => {
      ...
      for (let i = 0; i < mediaLength; i += 1) {
        next[i] = {
          splashes: i === 0 ? initialSplashes : 0,
          echoes: i === 0 ? initialEchoes : 0,
        };
      }
    ...
    const primaryMedia = (galleryMediaItems[0] ||
      item.media ||
      null) as Asset | null;
    ...
    const playbackLooksVideo =
      isExplicitVideo ||
      /(\.m3u8|\.mp4|\.mov|\.webm|\.mkv)(\?|$)/i.test(
        playbackUri.toLowerCase(),
      ) ||
    ...
    const audioOnlyPost =
      (isExplicitAudio ||
        ...
        (!item.playbackUrl &&
          !!primaryMedia &&
          isAudioAsset(primaryMedia) &&
          !hasVideoMedia &&
          !hasImageMedia)) &&
      !hasVideoMedia &&
      !hasImageMedia;
    const primaryVideoSource =
      item.playbackUrl && playbackLooksVideo
        ? String(item.playbackUrl)
        : String(primaryMedia?.uri || '');
    const fallbackVideoSource =
      item.playbackUrl &&
      playbackLooksVideo &&
      primaryMedia?.uri &&
      primaryMedia.uri !== item.playbackUrl
        ? String(primaryMedia.uri)
        : '';
    const videoSourceUri =
      (preferFallbackVideoSource
        ? fallbackVideoSource || primaryVideoSource
        : primaryVideoSource || fallbackVideoSource) || '';
    const feedVideoSource = useMemo(
      () => ({ uri: videoSourceUri }),
      [videoSourceUri],
    );
    // Always honor overlay audio on visual posts so image/video + audio plays as intended.
    const hasOverlayAudio =
      !!item.audio?.uri && !audioOnlyPost && (hasVideoMedia || hasImageMedia);
    const overlayReady = !hasOverlayAudio || overlayAudioLoaded;
    const audioPlaySynced =
      shouldPlay && item.id === activeVideoId && overlayReady;
    // Video must autoplay when this card is active; do not wait on overlay-audio progress
    // (that gate could deadlock playback when the hidden audio track never reports progress).
    const videoPlaySynced = shouldPlay && item.id === activeVideoId;
    ...
    const fallbackAwayText = (() => {
      const exact = formatPresenceLastSeenExact(
        userData[item.ownerUid || '']?.lastSeen || null,
      );
      return exact
        ? translate('feed.awaySince', { time: exact })
        : translate('feed.awayUnknown');
    ...
    const renderMoMoBadge = useCallback(() => null, []);
    ...
    const buildFeedTextOverlay = useCallback(
      (
        ...
        const overlayWidth = Math.max(
          96,
          Math.min(frameWidth * 0.8, frameWidth - 16),
        );
        return {
          containerStyle: {
            ...
            top: Math.max(
              ...
              Math.min(
                frameHeight - 56,
                Number(overlay?.y ?? 0.72) * frameHeight -
                  (overlay?.fontSize || 28),
              ),
            ),
            width: overlayWidth,
    ...
    const handleProfilePress = useCallback(() => {
      ...
      if (item.ownerUid === myUid) {
        ...
        onOpenCreatorProfile(
          item.ownerUid,
          item.authorName || item.user?.name || null,
        );
      }
    }, [
      item.authorName,
    ...
    const normalizeHandleLabel = useCallback(
      (raw?: string | null) => {
    ...
    const handleOnlineUserPress = useCallback(
    ...
    const openMediaViewer = useCallback(
      (startIndex: number) => {
        setViewerIndex(
          Math.max(0, Math.min(startIndex, galleryMediaItems.length - 1)),
        );
        setViewerZoom(1);
    ...
    const handleAddSplash = useCallback(() => {
      ...
      const updateFeeds = (feed: Vibe[]) =>
        feed.map(v => {
          ...
          const n =
            Math.max(0, Number(v.counts?.hugs ?? v.counts?.splashes ?? 0)) + 1;
        ...
        .catch(error => {
          ...
          const revertFeeds = (feed: Vibe[]) =>
            feed.map(v => {
              ...
              const n = Math.max(
                0,
                Number(v.counts?.hugs ?? v.counts?.splashes ?? 0) - 1,
              );
              return { ...v, counts: { ...v.counts, hugs: n, splashes: n } };
            });
        ...
        .catch(error => {
          ...
          const revertFeeds = (feed: Vibe[]) =>
            feed.map(v => {
              if (v.id !== item.id) return v;
              const n =
                Math.max(0, Number(v.counts?.hugs ?? v.counts?.splashes ?? 0)) +
                1;
              return { ...v, counts: { ...v.counts, hugs: n, splashes: n } };
            });
    ...
    const handleRemoveSplashForSelectedMedia = useCallback(() => {
      setMediaActionCounts(prev => {
        ...
        return {
          ...
          [selectedMediaIndex]: {
            ...current,
            splashes: Math.max(0, current.splashes - 1),
          },
        };
      });
      handleRemoveSplash();
    ...
    const handleLoadMoreEchoes = useCallback(() => {
      ...
      setEchoExpansionInProgress(prev => ({ ...prev, [item.id]: true }));
      setEchoesPageSize(prev => ({
        ...prev,
        [item.id]: (prev[item.id] || 5) + 5,
      }));
    ...
    const handleEchoReply = useCallback(
      (echo: any) => {
    ...
    const fetchRepliesForEcho = useCallback(
      async (echoId: string) => {
        try {
          ...
          >
            <Pressable
              onPress={() => {
                ...
                setActiveEchoActionId(prev =>
                  prev === echo.id ? null : echo.id,
                );
            ...
            >
              ...
              <Text style={{ color: 'black', fontSize: 14 }}>{echo.text}</Text>
              <Text style={{ color: 'gray', fontSize: 10 }}>
                {echo.createdAt
                  ? formatDefiniteTime(echo.createdAt)
                  : translate('feed.justNow')}
              </Text>
            </Pressable>
            ...
            {isExpanded && replies.length > 0 && (
              ...
              >
                {replies.map((reply, replyIdx) => (
                  ...
                  >
                    <Text
                      style={{ color: '#555', fontSize: 11, fontWeight: '600' }}
                    >
                    ...
                    <Text style={{ color: 'gray', fontSize: 9 }}>
                      {reply.createdAt
                        ? formatDefiniteTime(reply.createdAt)
                        : translate('feed.justNow')}
                    </Text>
                  </View>
                ))}
        ...
        <View style={styles.feedCard}>
                  ...
                  <View style={styles.profileColumn}>
                    ...
                    >
                      <ProfileAvatarWithCrew
                        ...
                        optimisticCrewCount={
                          optimisticCrewCounts[item.ownerUid!]
                        }
                      />
                    </Pressable>
                    <View style={styles.profileTextWrap}>
                      ...
                      {(() => {
                        ...
                        const rawTitle = String(titleToShow || '')
                          .trim()
                          .toLowerCase();
                        const badgeToShow =
                          rawTitle === 'fresh_face' ||
                          rawTitle.includes('fresh face')
                            ? '✨'
                            : rawTitle === 'rising_star' ||
                                rawTitle.includes('rising star')
                              ? '⭐'
                              : rawTitle === 'crowd_favorite' ||
                                  rawTitle.includes('crowd favorite')
                                ? '🔥'
                                : rawTitle === 'wave_king' ||
                                    rawTitle.includes('wave king')
                                  ? '👑'
                                  : rawTitle === 'trend_storm' ||
                                      rawTitle.includes('trend storm')
                                    ? '⚡'
                                    : rawTitle === 'ocean_legend' ||
                                        rawTitle.includes('ocean legend')
                                      ? '🦈'
                                      : String(titleToShow || '')
                                          .trim()
                                          .split(/\s+/)[0] || '';
                      ...
                      {(() => {
                        const isCurrentUserPost = item.ownerUid === myUid;
                        const bioToShow = isCurrentUserPost
                          ? profileBio
                          : userData[item.ownerUid!]?.bio;
                        return bioToShow ? (
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return (
                        <View
                          pointerEvents="none"
                          style={overlayRender.containerStyle}
                        >
                    ...
                    {hasOverlayAudio && RNVideo ? (
                      <RNVideo
                        ...
                        rate={Math.max(
                          0.5,
                          Math.min(2, Number(mediaEdits?.playbackRate || 1)),
                        )}
                        volume={Math.max(
                          0,
                          Math.min(2, Number(mediaEdits?.volumeBoost || 1)),
                        )}
                  ...
                  >
                    ...
                    {RNVideo ? (
                      <RNVideo
                        source={{
                          uri: String(
                            item.audio?.uri || primaryMedia?.uri || '',
                          ),
                        }}
                        audioOnly
          ...
          <View style={styles.posterActionWrap}>
            <PosterActionBar
              ...
              currentUserId={myUid || ''}
              splashesCount={
                hasMultiMediaGrid ? selectedMediaCounts.splashes : hugCountBase
              }
              echoesCount={
                hasMultiMediaGrid
                  ? selectedMediaCounts.echoes
              ...
              onAdd={
                hasMultiMediaGrid
                  ? handleAddSplashForSelectedMedia
                  : handleAddSplash
              }
              onRemove={
                hasMultiMediaGrid
                  ? handleRemoveSplashForSelectedMedia
                  : handleRemoveSplash
              }
              onEcho={
                hasMultiMediaGrid ? handleEchoForSelectedMedia : handleEcho
              }
              onPearl={handlePearl}
            ...
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.96)' }}>
              {galleryMediaItems[viewerIndex] ? (
                ...
                >
                  ...
                  {isImageAsset(galleryMediaItems[viewerIndex]) ? (
                  ...
                  ) : isVideoAsset(galleryMediaItems[viewerIndex]) &&
                    RNVideo ? (
                    <RNVideo
                      source={{
                        uri: String(galleryMediaItems[viewerIndex].uri),
                    ...
                    >
                      <Text style={{ fontSize: 42, marginBottom: 12 }}>
                        File
                      </Text>
                      <Text
                        style={{
                      ...
                      >
                        {galleryMediaItems[viewerIndex].fileName ||
                          'Attachment'}
                      </Text>
                    </View>
                  )}
                  <Text
                    style={{ color: 'rgba(255,255,255,0.82)', marginTop: 12 }}
                  >
                  ...
                  >
                    ...
                    {isImageAsset(galleryMediaItems[viewerIndex]) ? (
                      <Pressable
                        onPress={() =>
                          setViewerZoom(prev =>
                            Math.min(4, Number((prev + 0.5).toFixed(1))),
                          )
                        }
                    ...
                    {galleryMediaItems.length > 1 ? (
                      <Pressable
                        onPress={() => {
                          setViewerZoom(1);
                          setViewerIndex(prev =>
                            Math.min(galleryMediaItems.length - 1, prev + 1),
                          );
                        }}
                        ...
                        style={{
                          ...
                          borderRadius: 18,
                          backgroundColor:
                            viewerIndex >= galleryMediaItems.length - 1
                              ? 'rgba(255,255,255,0.08)'
                              : 'rgba(255,255,255,0.16)',
                        }}
                      >
          ...
          >
            ...
            {item.ownerUid !== myUid && (
              <Text
                style={[
                  styles.presenceText,
                  { color: isHereNow ? ui.colors.success : ui.colors.subtle },
                ]}
              >
                {isHereNow
                  ? translate('feed.hereNow')
                  : status || fallbackAwayText}
              </Text>
            )}
          </ScrollView>
              ...
              {postEchoLists[item.id] && postEchoLists[item.id].length > 0 && (
                <View style={{ marginTop: 10 }}>
                  {expandedEchoes[item.id]
                    ...
                    : (() => {
                        const topEcho = [...postEchoLists[item.id]].sort(
                          (a, b) => {
                            const hugDiff =
                              Number(b?.hugs || 0) - Number(a?.hugs || 0);
