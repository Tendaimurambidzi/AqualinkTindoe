...
import MoonNightMode from './src/components/MoonNightMode';
...
import {
  ensureSplash as ensureSplashService,
  removeSplash as removeSplashService,
} from './src/services/splashService';
import {
  joinCrew,
  leaveCrew,
  isInCrew,
...
// Brand animation removed per request; keep logo only on auth background
// Pre-resolve splash assets so Metro can validate paths at build time
// and avoid dynamic require resolution issues.
let splashMoon: any = null;
let splashFalcon: any = null;
let myLogo: any = null;
let splashSound: any = null;
try {
  splashMoon = require('./assets/moon.png');
} catch {}
try {
...
// Try to load map texture, but don't crash if it's missing.
let paperTexture: any = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  paperTexture = require('./assets/MAP.jpg');
} catch (e) {
  console.log('Could not load MAP.jpg from assets. Using fallback color.');
}
...
const SOUND_EFFECT_ASSETS: Record<number, number> = {
  1: require('./assets/downfall-3-208028.mp3'),
  2: require('./assets/large-underwater-explosion-190270.mp3'),
  3: require('./assets/sci-fi-sound-effect-designed-circuits-hum-10-200831.mp3'),
  4: require('./assets/falcon.mp3'),
};

const resolveAssetUri = (asset: number | string | undefined) => {
  try {
    return Image.resolveAssetSource(asset || 0)?.uri || null;
  } catch {
    return null;
  }
};
// Optional network info (lazy required below if installed)
...
// Silence warnings/logs in production so dev debugging noise never reaches users
try {
  if (!__DEV__) {
    try {
    ...
    const noop = () => {};
    // Best-effort: prevent logs from appearing or being forwarded in release builds
    // (RedBox/YellowBox are already disabled in release by RN)
    // This ensures any lingering console usage stays local to dev only.
    // eslint-disable-next-line @typescript-eslint/no-empty-function
    // @ts-ignore
    console.log = noop; // no-op
    // @ts-ignore
    console.warn = noop;
    // @ts-ignore
    console.error = noop;
...
// Disable forced sign-out on start to avoid churn/crashes on second open
const FORCE_SIGN_OUT_ON_START = false;

const Stack = createNativeStackNavigator();

// Simple error boundary to prevent hard crashes in production builds
...
/**
 * App.tsx — Ocean App UI Scaffold (SafeAreaProvider)
 * ---------------------------------------------------
 * Change request:
 *  • Restore icons ABOVE the words for the top functions.
 *  • Remove the rectangular chip backgrounds; leave the words "in the open" but still clickable.
 *  • Keep all prior updates (MY SHORE on right half under camera, SafeAreaProvider, layout, counters, bubbles, modals).
 */

const { width: SCREEN_WIDTH, height: SCREEN_HEIGHT } = Dimensions.get('window');
const STATS_OVERLAY_HEIGHT = 70;
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#001529' },
  topStrip: {
    position: 'absolute',
    top: 0,
    left: 0,
  ...
  videoSpaceInner: { flex: 1, justifyContent: 'center' },
  videoHint: { color: 'rgba(255,255,255,0.5)', fontSize: 12 },
  mediaPreview: { flex: 1, width: '100%', resizeMode: 'contain' },
  postedWaveContainer: { flex: 1, width: '100%', alignItems: 'center', justifyContent: 'center' },
  postedWaveMedia: {
    position: 'absolute',
    top: 0,
    left: 0,
...
const editorStyles = StyleSheet.create({
  editorRoot: { flex: 1, backgroundColor: '#001529' },
  stage: { flex: 1, position: 'relative', backgroundColor: '#001529' },
  editorContainer: {
...
const InnerApp: React.FC<InnerAppProps> = ({ allowPlayback = true }) => {
  ...
  const splashDisplayCount = hasSplashed ? Math.max(1, splashes) : splashes;
  ...
  const backendSearchBase = useMemo(() => {
    try {
      const cfg = require('./liveConfig').default;
      return (
  ...
  const normalizeSearchTerm = (input: string) => {
  ...
  const searchViaFirestore = useCallback(async (term: string): Promise<SearchResult[]> => {
    ...
    const addUserDoc = (doc: any) => {
      ...
      results.push({
        kind: 'user',
        id: uid,
        label: data.displayName || data.userName || data.username || '@drifter',
        extra: {
          bio: data.bio || '',
    ...
    try {
      ...
      waveSnap.docs.forEach(doc => {
        ...
        results.push({
          kind: 'wave',
          id,
          label: data.captionText || data.caption || data.authorName || 'Wave',
          extra: {
    ...
    async (term: string): Promise<SearchResult[]> => {
      ...
      if (backendSearchBase) {
        ...
        try {
          ...
          const wavesData = wavesResp.ok ? (await wavesResp.json()).waves || [] : [];
          const results: SearchResult[] = [];
          ...
          usersData.forEach((user: any) => {
            ...
            results.push({
              kind: 'user',
              id: uid,
              label: user.displayName || user.userName || user.username || '@drifter',
              extra: { ...user },
            });
          });
          wavesData.forEach((wave: any) => {
            ...
            results.push({
              kind: 'wave',
              id,
              label: wave.caption || wave.title || wave.authorName || 'Wave',
              extra: { ...wave },
            });
          });
          if (results.length > 0) {
      ...
      return searchViaFirestore(normalized);
    },
  ...
  const runDeepSearch = useCallback(async () => {
    ...
    if (!term) {
      Alert.alert('Search', 'Please enter a username or keyword.');
      return;
    }
    ...
    try {
      ...
      if (results.length === 0) {
        setDeepSearchError(`No results matching "${term}"`);
      }
      ...
      console.warn('Deep search failed', err);
      setDeepSearchError('Search failed');
    } finally {
  ...
  useEffect(() => {
    loadDriftWatchers();
    const sub = auth().onAuthStateChanged(() => loadDriftWatchers());
    return () => sub && sub();
  }, [loadDriftWatchers]);
  ...
  // Prefer SurfaceView for certain devices (e.g., Samsung A12) to avoid decoder issues
  // Device-specific video surface selection: prefer SurfaceView on Samsung A12, TextureView otherwise
  const useTextureForVideo = useMemo(() => {
    if (Platform.OS !== 'android') return undefined;
    try {
      const constants: any = (Platform as any).constants || {};
      const brand = String(
        constants.Brand || constants.Manufacturer || '',
      ).toLowerCase();
      const model = String(constants.Model || '').toLowerCase();
      const key = `${brand} ${model}`;
      if (
        key.includes('samsung') &&
        (key.includes('a12') || key.includes('sm-a125'))
      ) {
        return false; // prefer SurfaceView on Samsung A12 family
      }
    } catch {}
    return true; // default TextureView on Android
  }, []);
  ...
  // Use selected feed for rendering
  const displayFeed = showPublicFeed ? publicFeed : wavesFeed;
  const currentWave =
  ...
  const wavesCountDisplay = useMemo(
    () =>
      typeof myWaveCount === 'number' ? myWaveCount : wavesFeed.length,
    [myWaveCount, wavesFeed.length],
  );
  const totalSplashesOnMyWaves = useMemo(
    () =>
      Math.max(
        0,
        wavesFeed.reduce(
          (sum, wave) => sum + (waveStats[wave.id]?.splashes || 0),
          0,
        ),
      ),
    [wavesFeed, waveStats],
  ...
  const statsEntries: StatEntry[] = useMemo(
    () => [
      ...
      { key: 'splashes', label: 'Splashes', value: totalSplashesOnMyWaves },
      {
        key: 'hugs',
        label: 'Hugs',
        value: userStats.hugsMade,
        onPress: handleHugsPress,
      },
      {
        key: 'echoes',
        label: 'Echoes',
        value: totalEchoesOnMyWaves,
        onPress: handleEchoesPress,
      },
    ],
    [
      wavesCountDisplay,
    ...
    async (label: string) => {
      ...
      if (label === 'Save to device') {
        ...
        try {
          ...
          if (downloadUrl) {
            // Open the video URL - browser/system will handle download
            // Note: SPL watermark feature requires additional setup (see WATERMARK_SETUP.md)
            Linking.openURL(String(downloadUrl));
            Alert.alert(
              'Opening Video',
              "Video will open in your browser or video player. Use your browser's download option to save it.\n\nNote: Install watermark dependencies for SPL-branded downloads (see WATERMARK_SETUP.md)",
              [{ text: 'OK' }],
            );
          } else {
  ...
  const deleteWave = (waveId: string) => {
    ...
    // You might want to also delete from remote storage here
  };
  ...
  const onSendEcho = async () => {
    ...
    try {
      ...
      setMyEcho({ text });
      console.log('[ECHO] Echo sent successfully!');

      // Close the echo modal IMMEDIATELY to show the animation
      setShowEchoes(false);

      // Ocean-themed echo notification with ripple animation
      console.log('[ECHO] Triggering ocean ripple animation...');
      runEchoRippleAnimation('Echo cast, drifting into the open sea!');

      // Removed duplicate Alert - notification already shown above
      try {
        recordPingEvent('echo', currentWave.id, { text });
      } catch {}
    } catch (e: any) {
      console.error('[ECHO] Send echo failed:', e);
  ...
  const handleJoinCrew = async (targetUid: string, targetName?: string) => {
    ...
    try {
      ...
      notifyError('Could not join crew right now');
    } finally {
  ...
  const handleBlockUser = async (targetUid: string, targetName?: string) => {
    try {
      ...
      try {
        const cfg = require('./liveConfig').default;
        const backendUrl = cfg?.BACKEND_BASE_URL || cfg?.USER_MGMT_ENDPOINT_BASE;
        if (backendUrl) {
        ...
        console.log('Backend block update failed:', backendErr);
      }

      notifySuccess(`Blocked ${targetName || 'user'}`);
    } catch (e) {
      console.error('Block user error:', e);
      notifyError('Could not block user');
    }
  };
  ...
  const handleRemoveFromDrift = async (targetUid: string, targetName?: string) => {
    try {
      ...
      notifySuccess(`Removed ${targetName || 'user'} from drift`);
    } catch (e) {
      console.error('Remove user error:', e);
      notifyError('Could not remove user');
    }
  };
  ...
  const formatPingMessage = useCallback(
    (ping: Ping) => {
      ...
      switch (ping.type) {
        case 'splash':
          // Check if it's a hug or regular splash
          if (ping.splashType === 'octopus_hug') {
            return `${actor} hugged your wave`;
          }
          return `${actor} splashed your wave`;
        case 'echo':
          return `${actor} echoed your wave`;
        case 'follow':
  ...
  const pickAudioWithDocumentPicker = async () => {
    // Ask for appropriate runtime permission on Android before proceeding
    const ok = await ensureAudioPermission();
    if (!ok) {
      Alert.alert(
        'Permission denied',
        'Unable to access audio without permission.',
      );
      return;
    }
    let DP: any | null = null;
    try {
      // Lazy require so the app won’t crash if the module isn’t installed yet
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        DP = require('@react-native-documents/picker');
      }
    } catch (e) {
      DP = null;
    }
    ...
    try {
      ...
      if (file?.uri || file?.fileCopyUri) {
        let playableUri = file?.fileCopyUri || file?.uri;
        // If still content:// (e.g., ExternalStorageProvider), force a local copy
        if (!/^file:/.test(String(playableUri))) {
          try {
            ...
            await RNFS.copyFile(String(playableUri), dest);
            playableUri = `file://${dest}`;
          } catch (copyErr) {
            console.warn('Audio copy fallback failed', copyErr);
          }
        }
        if (/^file:/.test(String(playableUri))) {
          ...
          try {
            ...
            setAttachedAudio({
              uri: outUri,
              name: (file.name || 'audio').replace(/\.[^.]+$/, '') + '.m4a',
            });
          } catch {
  ...
  const onPostWave = async () => {
    ...
    try {
      ...
      if (storageMod && firestoreMod && authMod) {
        ...
        await storageMod()
          .ref(filePath)
          .putFile(localPath, { contentType: videoContentType });
        ...
        if (attachedAudio?.uri) {
          if (isHttp(attachedAudio.uri)) {
            ...
            if (/^content:/.test(String(attachedAudio.uri))) {
              try {
                const RNFS = require('react-native-fs');
                ...
                await RNFS.copyFile(String(attachedAudio.uri), copyDest);
                audioLocal =
                  Platform.OS === 'android' ? copyDest : `file://${copyDest}`;
              } catch (e) {
        ...
        serverDocId = docRef?.id || null;
        // Notify backend that a wave was posted
        try {
          const cfgLocal = (() => {
            try {
              return require('./liveConfig');
  ...
  if (!isFocused) {
    ...
    >
                    ...
                    !(videoErrorMap || {})[item.id] ? (
                      item.playbackUrl ? (
                        ...
                        <RNVideo
                          ...
                          poster={String(item.playbackUrl)}
      ...
      <View style={[styles.rightBubbles, { top: rightBubblesTop }]}>
        ...
        <View style={styles.avatarStack}>
          {crew.slice(0, 3).map((member, i) => (
            <CrabWalkBadge key={member.id}>
              <Pressable
                style={[styles.crewBubble, i > 0 ? { marginTop: -12 } : null]}
                onPress={() => requestToDriftForLiveId(member.id, member.name)}
              >
                {member.avatar ? (
                  <Image
          ...
          {crew.length > 3 && (
            <Pressable
              style={styles.avatarStackMore}
              onPress={() =>
                Alert.alert(
                  'More Waves',
                  'More waves have been posted by the crew.',
                )
              }
            >
              <Text style={styles.avatarStackMoreText}>+</Text>
            </Pressable>
          )}
        </View>

        {/* Recent Splashers Stack (third stack) - merged into crew stack with overlap */}
        {(recentSplashers.length === 0
          ? [{ id: 'placeholder', name: 'D', avatar: null }]
          : recentSplashers.slice(0, 3)
        ).map((spl: any, i: number) => (
          <View
            key={spl.id}
            style={[
              styles.crewBubble,
              { marginTop: -12 },
              {
                ...
                opacity: spl.id === 'placeholder' ? 0.4 : 1,
              },
            ]}
          >
            {spl.avatar ? (
              <Image source={{ uri: spl.avatar }} style={styles.crewAvatar} />
            ) : (
              <Text style={styles.crewInitial}>
                {spl.name ? spl.name.charAt(0).toUpperCase() : 'D'}
              </Text>
            )}
          </View>
        ))}
        {recentSplashers.length > 3 && (
          <View style={[styles.avatarStackMore, { marginTop: -12 }]}>
            <Text style={styles.avatarStackMoreText}>
              +{recentSplashers.length - 3}
            </Text>
          </View>
        )}
          ...
          >
            ...
            {isBottomBarExpanded && !isSwiping && (
              ...
              >
                ...
                >
                  {/* Show 1 drop when not splashed, 3 drops when splashed */}
                  {hasSplashed ? (
                    <View
                      style={{
                        flexDirection: 'row',
                      ...
                      <Animated.Text
                        style={[
                          styles.bottomBarIcon,
                          {
                            ...
                            left: 2,
                            top: 2,
                            opacity: smallDropsOpacity,
                            transform: [
                              { scale: splashAnimation },
                              { scale: smallDropsScale },
                            ],
                          },
                        ]}
                      >
                  ...
                  <Text
                    style={[
                      ...
                      hasSplashed && { color: '#00C2FF' },
                    ]}
                  >
                    {hasSplashed ? 'Splashed' : 'Splashes'}
                  </Text>
      ...
      {(() => {
        try {
          return autoNightMode ? <MoonNightMode /> : null;
        } catch (err) {
          ...
          >
            ...
            <View style={styles.statsOverlay}>
              <View style={styles.statsOverlayRow}>
                {statsEntries.map(entry => {
                  const content = (
                    <>
                      <Text style={styles.statNumber}>
                        {formatCount(Math.max(0, entry.value))}
                      </Text>
                  ...
                  return (
                    <View key={entry.key} style={styles.statTile}>
                      {content}
                    </View>
                  );
                })}
            ...
            <View
              style={{
                flex: 1,
                flexDirection: 'row',
                marginTop: STATS_OVERLAY_HEIGHT + 8,
              }}
            >
              {/* Left Page: Profile Summary + Actions */}
              <View style={[styles.logbookPage, { paddingTop: 16 }]}>
              {/* My Waves entry (DRIFT removed here by request) */}
              <Pressable
                style={[styles.logbookAction, { marginTop: 24 }]}
                onPress={() => {
                  setShowProfile(false);
                  setShowMyWaves(true);
                }}
          ...
          >
            ...
            <UserSearch
              onUserSelect={async (user) => {
                // When user selected from search, show their info
                const result = {
                  kind: 'user' as 'user',
                  id: user.uid,
                  label: user.displayName,
                  extra: { bio: '', photoURL: user.photoURL },
                };
                setDeepResults([result]);
                
                // Load crew status
                try {
                  const inCrew = await isInCrew(user.uid);
                  setIsInUserCrew(prev => ({ ...prev, [user.uid]: inCrew }));
                } catch {}
              }}
              onJoinCrew={user =>
                handleJoinCrew(user.uid, user.displayName || user.username)
              }
              onInviteToDrift={user =>
                handleUserAction(
                  'inviteToDrift',
                  user.uid,
            ...
            />
            <Text style={[styles.hint, { marginTop: 16, marginBottom: 8 }]}>
              Or search by username:
            </Text>
            <TextInput
              value={deepQuery}
              onChangeText={v => setDeepQuery(v)}
              placeholder="Search users or waves..."
              placeholderTextColor="rgba(255,255,255,0.4)"
              ...
              {deepResults.map(result => (
                <View key={result.kind + result.id}>
                  <Pressable
                    ...
                    onPress={() => {
                      if (result.kind === 'wave') {
                        handleDeepWaveSelect(result);
                      }
                    }}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={styles.pingText}>
                        {result.kind === 'user' ? '👤 ' : '🌊 '} {result.label}
                      </Text>
                      {result.kind === 'user' && result.extra?.bio && (
                        <Text
                          style={[
                            styles.hint,
                        ...
                        >
                          {String(result.extra.bio).slice(0, 80)}
                        </Text>
                      )}
                    </View>
                    {result.kind === 'wave' && (
                  ...
                  {result.kind === 'user' && (
                    <View style={styles.deepResultActions}>
                      <Pressable
                        style={[
                          styles.deepResultActionButton,
                      ...
                      >
                        <Text
                          style={[
                            styles.deepResultActionText,
                            { color: isInUserCrew[result.id] ? 'white' : '#00C2FF' },
                          ]}
                        >
                      ...
                      <Pressable
                        style={styles.deepResultActionButton}
                        onPress={() =>
                          inviteCoHostById(result.id, result.label)
                        }
                      >
                        <Text style={styles.deepResultActionText}>Invite Drift</Text>
                      </Pressable>
                      {result.extra?.liveId && (
                        <Pressable
                          style={styles.deepResultActionButton}
                        ...
                        >
                          <Text style={styles.deepResultActionText}>
                            Request Drift
                          </Text>
                        </Pressable>
                      )}
            ...
            {showCaptionInput ? (
              <PanGestureHandler
                onGestureEvent={Animated.event(
                  [
                    {
                      nativeEvent: {
                        translationX: captionDrag.x,
                        translationY: captionDrag.y,
                      },
                    },
                  ],
                ...
                onHandlerStateChange={e => {
                  if (e.nativeEvent.state === State.END) {
                    ...
                    const baseX = captionPos?.x ?? 24;
                    const baseY =
                      captionPos?.y ?? Math.floor(stageSize.h * 0.35);
                    const nextX = Math.max(
                      0,
                      Math.min(stageSize.w - 10, baseX + (translationX || 0)),
                    );
                    const nextY = Math.max(
                      0,
                      Math.min(stageSize.h - 10, baseY + (translationY || 0)),
                    );
                    setCaptionPos({ x: nextX, y: nextY });
                    captionDrag.extractOffset();
                  }
                }}
              >
                <Animated.View
                  style={[
                    editorStyles.draggableCaptionContainer,
                    { transform: captionDrag.getTranslateTransform() },
                  ]}
                >
                  <TextInput
              ...
              !!captionPos && (
                <Text
                  style={[
                    styles.postedWaveCaption,
                    { left: captionPos.x, top: captionPos.y },
                  ]}
                >
                  {captionText}
                </Text>
                ...
                <>
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      setWaveOptionsTarget(null);
                      if (isInUserCrew[targetUid]) {
                        handleLeaveCrew(targetUid);
                      } else {
                        handleJoinCrew(targetUid);
                      }
                    }}
                  ...
                  >
                    <Text style={styles.waveOptionsItemTitle}>
                      {crewLoading
                        ? 'Loading...'
                        : isInUserCrew[waveOptionsTarget.ownerUid!]
                        ? 'Leave Crew'
                        : 'Join Crew'}
                    </Text>
                    <Text style={styles.waveOptionsItemDescription}>
                      {isInUserCrew[waveOptionsTarget.ownerUid!]
                        ? 'Remove this captain from your boarding list'
                  ...
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      Alert.alert(
                        'Remove from Drift',
                        `Remove ${targetName || 'this user'} from your drift? This hides their content.`,
                        [
                          { text: 'Cancel', style: 'cancel' },
                          {
                            text: 'Remove',
                            style: 'destructive',
                            onPress: () => handleRemoveFromDrift(targetUid, targetName),
                          },
                        ]
                      );
                    }}
                  >
                    <Text style={styles.waveOptionsItemTitle}>Remove from Drift</Text>
                    <Text style={styles.waveOptionsItemDescription}>
                      Hide this user's waves from your feed
                    </Text>
                  </Pressable>
                </>
              )}
...
const LiveStreamModal = ({
  ...
  const createLivePoll = () => {
    ...
    broadcastPollToViewers(question);
    Alert.alert('Poll', 'Created a sample poll.');
  };

  const handleSetLiveGoal = () => {
    setLiveGoal({ target: 100, current: 0 });
    Alert.alert('Live Goal', 'Set a sample goal of 100.');
  };
  ...
  const playSoundEffect = (soundId: number) => {
    const asset = SOUND_EFFECT_ASSETS[soundId];
    const uri = resolveAssetUri(asset);
    if (!asset || !uri) {
      console.warn('Missing sound asset for id', soundId);
      return;
    }
    try {
      engineRef.current?.startAudioMixing?.({
        filePath: uri,
        loop: false,
        replace: true,
        cycle: 1,
      ...
      console.warn('Sound mixing failed', error);
    }
  };

  const showSoundBoard = () => {
    const sounds = [
      { id: 1, name: 'Applause', icon: '👏' },
      { id: 2, name: 'Drumroll', icon: '🥁' },
      { id: 3, name: 'Airhorn', icon: '📯' },
      { id: 4, name: 'Bell', icon: '🔔' },
    ];
    Alert.alert(
      'Sound Effects',
  ...
  const UserManagementPanel = ({
    ...
    const filtered = viewers.filter(u =>
      (u.label || u.username || '').toLowerCase().includes(searchQuery.toLowerCase()),
    );

    const { isInUserCrew } = useCrewStatus(panelSearchResults.map(r => r.id));
    ...
    const getUserActions = (user: any) => {
      ...
      const restrictive = [
        ...
        { label: 'Timeout', action: 'timeout', icon: 'dYZ?' },
      ];
      const crewActions = [
        {
          label: isInUserCrew[user.id] ? 'Remove from Crew' : 'Join Crew',
          action: isInUserCrew[user.id] ? 'removeFromCrew' : 'join',
          icon: 'dY?',
        },
    ...
    const displayUsers = shouldShowSearchResults ? panelSearchResults : filtered;
    ...
    try {
      ...
      if (userCredential.user) {
        ...
        await firestore().collection('users').doc(userCredential.user.uid).set({
          ...
          username_lc: trimmedUsername.toLowerCase(), // for case-insensitive checks
          email: trimmedEmail,
          createdAt: firestore.FieldValue.serverTimestamp(),
        });
...
function WelcomeAnimationScreen({ navigation }: any) {
  ...
  return (
    <Pressable
      ...
      onPress={handleSkip}
    >
      {/* Floating water animation background */}
      <FloatingWaterAnimation />
      ...
      >
        ...
        >
          {`Make a splash. Get seen${'.'.repeat(dots)}!`}
        </Animated.Text>
      </Animated.View>
      
      {/* Swimming fish across the water - below text */}
      <View style={{ position: 'absolute', top: '45%', width: '100%', zIndex: 5 }}>
        <SwimmingFishLoader />
      </View>
...
const App: React.FC = () => {
  ...
  // Show splash only on cold start (first mount). Do not replay on resume.
  // No AppState listener here on purpose.
  useEffect(() => {
